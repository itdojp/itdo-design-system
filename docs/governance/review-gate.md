...
- `Review Gate / gate` status check
  - Requires at least one review from configured Copilot actors.
  - Fails when Copilot review threads include unresolved conversations.
...
- Branch protection preset
  ...
  - Enforces strict status checks and admin enforcement.
...
- Human approval count is not enforced in the preset. Human review can be added separately if needed.
